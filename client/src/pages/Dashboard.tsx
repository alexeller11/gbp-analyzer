...
import { Search, Loader2, Camera, FileText, RefreshCw, Zap, CheckCircle2, AlertTriangle, TrendingUp, Bell, Star, ArrowRight, XCircle } from "lucide-react";
...
function ProfileAlert({ profile }: { profile: any }) {
  if ((profile.avgRating || 0) < 4.0 && (profile.totalReviews || 0) > 5) {
    return <span className="text-[10px] bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full font-medium">⚠ Nota baixa</span>;
  }
  if (!profile.description || !profile.website) {
    return <span className="text-[10px] bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded-full font-medium">📝 Perfil incompleto</span>;
  }
  return null;
}
...
export default function Dashboard() {
  ...
  const [sortBy, setSortBy] = useState<"score" | "rating" | "attention">("attention");
  ...
  const handleAutoImport = async (silent = false) => {
    ...
    setSyncDone(null); setSyncError("");
    try {
      setSyncStep("Buscando perfis e avaliações...");
      const result = await autoImportMutation.mutateAsync();
      await refetch();
      setSyncDone(result);
      toast.success((result as any).fallback ? `Sincronizado · ${result.reviewsSynced} avaliações` : `✅ ${result.imported} importados · ${result.reviewsSynced} avaliações`);
    } catch (e: any) {
  ...
  useEffect(() => {
    if (!isLoading && profiles !== undefined) {
      if (profiles.length === 0 && !autoImportFired.current) {
  ...
  const sortedFiltered = [...(profiles || [])]
    .filter(p => p.name?.toLowerCase().includes(search.toLowerCase()) || p.address?.toLowerCase().includes(search.toLowerCase()) || p.category?.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => {
      if (sortBy === "score") return calcScore(b) - calcScore(a);
      if (sortBy === "rating") return (b.avgRating || 0) - (a.avgRating || 0);
      // "attention" = pior score primeiro
  ...
  const needsAttention = (profiles || []).filter(p => calcScore(p) < 60 || (p.avgRating || 0) < 4.0).length;
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Google Business Profile · {profiles?.length || 0} perfis</p>
          </div>
        ...
        {/* Onboarding */}
        {onboardingStep === 1 && (profiles?.length === 0) && !syncing && (
          <Card className="border-blue-300 bg-gradient-to-br from-blue-50 to-white">
            <CardContent className="pt-6 pb-6">
              <div className="text-center mb-5">
                <div className="text-4xl mb-2">🚀</div>
                <h2 className="text-xl font-bold">Bem-vindo ao GBP Analyzer!</h2>
                <p className="text-sm text-muted-foreground mt-1">Vamos configurar sua conta em 3 passos</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
                {[
                  { n: 1, title: "Importar Perfil", desc: "Adicione pelo link do Google Maps", done: false },
                  { n: 2, title: "Primeira Sync", desc: "Puxamos avaliações e dados", done: false },
                  { n: 3, title: "Ver Score", desc: "Análise completa do seu GBP", done: false },
                ].map(s => (
                  <div key={s.n} className="flex-1 flex items-start gap-3 p-3 bg-white rounded-xl border">
              ...
              <div className="flex gap-2 justify-center mt-5">
                <Button onClick={() => handleAutoImport(false)} disabled={syncing} className="gap-2">
                  <Zap className="w-4 h-4" /> Tentar importar automaticamente
                </Button>
                <ImportProfileDialog onSuccess={() => { refetch(); setOnboardingStep(0); }} />
              </div>
              <button className="block mx-auto mt-3 text-xs text-muted-foreground hover:underline" onClick={() => setOnboardingStep(0)}>Fechar</button>
            </CardContent>
          </Card>
        )}
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-xl p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-orange-600 text-xs mt-1 mb-2">Ative no <a href="https://console.cloud.google.com/apis/library" target="_blank" rel="noreferrer" className="underline font-medium">Google Cloud Console</a> → <strong>My Business Account Management API</strong> + <strong>My Business Business Information API</strong></p>
                <p className="text-orange-600 text-xs">Enquanto isso, use <strong>"Adicionar Perfil"</strong> com o link do Google Maps.</p>
              </div>
              <button onClick={() => setSyncError("")}><XCircle className="w-4 h-4 text-orange-400 hover:text-orange-600" /></button>
            </div>
          </div>
        )}
          ...
          <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <p className="text-green-800 text-sm font-medium">
                {(syncDone as any).fallback ? `Sincronizado · ${syncDone.reviewsSynced} avaliações` : `${syncDone.imported} novos · ${syncDone.skipped} atualizados · ${syncDone.reviewsSynced} avaliações`}
              </p>
            </div>
        ...
        {profiles && profiles.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: "Perfis", value: profiles.length, color: "text-blue-600", icon: "🏢" },
              { label: "Score Médio", value: avgScore, color: avgScore >= 75 ? "text-green-600" : avgScore >= 50 ? "text-yellow-600" : "text-red-500", icon: "🎯" },
              { label: "Nota Média", value: avgRating, color: "text-yellow-600", icon: "⭐" },
              { label: "Precisam atenção", value: needsAttention, color: needsAttention > 0 ? "text-red-500" : "text-green-600", icon: needsAttention > 0 ? "🔴" : "✅" },
            ].map(m => (
            ...
            <div className="flex gap-1">
              {[
                ...
                <Button key={s.key} variant={sortBy === s.key ? "default" : "outline"} size="sm"
                  className="text-xs h-9" onClick={() => setSortBy(s.key as any)}>
                  {s.label}
                </Button>
              ))}
            </div>
        ...
        {(isLoading || (syncing && !profiles?.length)) ? (
          ...
          <Card><CardContent className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Sincronize pelo botão acima ou adicione manualmente</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {sortedFiltered.map((profile, idx) => {
              const score = calcScore(profile);
              const rankBadge = sortBy === "attention" && idx < 3 && score < 60;
              return (
                <Card key={profile.id}
                  className={`cursor-pointer hover:shadow-lg transition-all group ${score < 50 ? "border-red-200" : score < 70 ? "border-yellow-200" : "hover:border-blue-300"}`}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0 pr-3">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {profile.isVerified && <span className="text-[10px] text-green-600 font-medium flex-shrink-0">✓</span>}
                        </div>
                  ...
                  <CardContent className="space-y-2 pt-0">
                    ...
                    <div className="flex items-center justify-between">
                      {profile.lastSyncAt && <p className="text-[10px] text-muted-foreground">Sync: {new Date(profile.lastSyncAt).toLocaleDateString("pt-BR")}</p>}
                      <span className="text-[10px] text-blue-500 group-hover:underline ml-auto flex items-center gap-0.5">Ver detalhes <ArrowRight className="w-2.5 h-2.5" /></span>
                    </div>
                  </CardContent>
                </Card>
              );
